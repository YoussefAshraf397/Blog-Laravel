...
<div class="row">
    <div class="col-lg-9 col-12 mx-auto">
        ...
        <div class="card card-body mt-4">
            <form action="{{ route('admin.place-type.update',$placeType->id) }}" method="POST"
                ...
                @php
                    ...
                    $descriptionArray = json_decode($placeType->details, true);
                @endphp
                <h6 class="mb-0">Edit Place Type</h6>
                <p class="text-sm mb-0">Edit the place type</p>
                ...
                <select name="status" class="multisteps-form__select form-control">
                    <option value="active" {{$placeType->status == 'active' ? 'selected="selected"' : '' }}>active</option>
                    <option value="suspend" {{$placeType->status == 'suspend' ? 'selected="selected"' : '' }}>suspend</option>
                </select>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" name="button" class="btn bg-gradient-info m-0 ms-2">Update Place Type</button>
                </div>
            </form>
        </div>
    </div>
