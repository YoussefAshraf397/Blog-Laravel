    ...
    <div class="col-lg-9 col-12 mx-auto">
        ...
        <div class="card card-body mt-4">
            <form action="{{ route('admin.attribute.update',$attribute->id) }}" method="POST"
                ...
                @endphp
                <h6 class="mb-0">Edit Attribute</h6>
                <p class="text-sm mb-0">Edit the attribute</p>
                ...
                <div class="row mt-3">
                    <div class="col-12 col-sm-6">
                        <label>Minimum</label>
                        <input name="minimum" class="multisteps-form__input form-control" type="text" value="{{$attribute->minimum}}" />
                    </div>
                    <div class="col-12 col-sm-6 mt-3 mt-sm-0">
                        <label>Maximum</label>
                        <input name="maximum" class="multisteps-form__input form-control" type="text" value="{{$attribute->maximum}}" />
                    </div>
                </div>
                <br>
                <div class="row">
                    <div class="col-12 col-sm-12">
                        ...
                        <select name="type" class="multisteps-form__select form-control">
                            @foreach($attributesTypes as $attributeType)
                                <option  {{$attributeType == $attribute->type ? 'selected="selected"' : ''}} value="{{ $attributeType }}">{{ $attributeType}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" name="button" class="btn bg-gradient-info m-0 ms-2">Update Attribute</button>
                </div>
            </form>
        </div>
    </div>
